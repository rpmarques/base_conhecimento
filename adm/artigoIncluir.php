...
if (isset($_POST)) {
    ...
    if (isset($_POST['titulo']) && isset($_POST['descricao']) && $_POST['categoria_id']) {
        if ($_FILES['arquivo']['name']<>''){
            $wAuxArquivo  = $_FILES['arquivo']['name'];
            $wExtensao    = str_replace('.', '', strstr($_FILES['arquivo']['name'], '.'));
            $wNomeArquivo = 'art_'.md5(time()).'.'.$wExtensao;
        }else{
            $wAuxArquivo='';
        }
        $retorno = $objArtigos->insert($_POST['titulo'], $_POST['descricao'], $_POST['categoria_id'],$wNomeArquivo,$wAuxArquivo);
        if ($retorno) {
            //VERIFICA SE TEM ARQUIVO PARA ENVIAR
            if ($wAuxArquivo){
                $wDiretorio='../arquivos/';
                if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $wDiretorio . $wNomeArquivo)) {
                    escreve( "UPLOAD DE ARQUIVO REALIZADO COM SUCESSO!");
                  }else{
                      escreve($_FILES['arquivo']['error']);
                  }
            }
